...
const fetchPrograms = async () => {
    const res = await request.get('pain-relief-programs');
    if (res.status) {
...
const fetchStatuses = async () => {
    const res = await request.get('donation-statuses');
    if (res.status) {
...
const search = async () => {
    const res = await request.post('pain-relief-donations/search', options.value);
    if (res.status) {
...
const get = async (e) => {
    ...
    try {
        const res = await request.post('pain-relief-donations/paginate', options.value);
        if (res.status) {
            items.value = res.data;
            total.value = res.tot;
        }
...
async function saveItem() {
    ...
    if (!item.value.donor_id || !item.value.amount || !item.value.status) {
    ...
    if (item.value.program_id) formData.append('program_id', item.value.program_id);
    if (imageFile.value) {
    ...
    try {
        const res = await request.post('pain-relief-donations', formData);
        if (res.status) {
...
async function edit() {
    ...
    try {
        const res = await request.put(`pain-relief-donations/${item.value.id}`, formData);
        if (res.status) {
...
async function deleteItem() {
    loading.value = true;
    try {
        const res = await request.delete(`pain-relief-donations/${item.value?.id}`);
        if (res.status) {
...
async function deleteSelectedItems() {
    ...
    loading.value = true;
    try {
        const res = await request.delete('pain-relief-donations/bulk', { ids: items });
        if (res.status) {
...
<template>
        ...
        <DataTable ref="dt" v-model:selection="selectedItems" :value="items" dataKey="id" :rows="options.limit" :filters="filters" :totalRecords="total" :loading="loading">
            <template #header>
                <div class="flex flex-wrap gap-2 items-center justify-between">
                    <h4 class="m-0">إدارة التبرعات لتخفيف الألم</h4>
                    <IconField>
            ...
            <Column field="donor.name" header="المتبرع" sortable style="min-width: 12rem">
                <template #body="slotProps">
                    {{ slotProps.data.donation ? slotProps.data.donation?.user?.name : '-' }}
                </template>
            </Column>
            <Column field="program.name" header="البرنامج" sortable style="min-width: 12rem">
                <template #body="slotProps">
                    {{ slotProps.data.program ? slotProps.data.program.name : '-' }}
                </template>
            </Column>
            ...
            <Column field="image_url" header="الصورة" style="min-width: 10rem">
                <template #body="slotProps">
                    <img v-if="slotProps.data.image_url" :src="slotProps.data.image_url" alt="Donation Image" style="width: 50px; height: auto" />
                    <span v-else>-</span>
                </template>
            </Column>
    ...
    <Dialog v-model:visible="itemDialog" :style="{ width: '600px' }" header="تبرع جديد" :modal="true" style="direction: rtl; text-align: right">
        <div class="flex flex-col gap-6">
            ...
            <div>
                <label for="program_id" class="block font-bold mb-2">البرنامج</label>
                <Dropdown id="program_id" v-model="item.program_id" :options="programs" optionLabel="name" optionValue="id" placeholder="اختر برنامج" class="w-full" />
            </div>
            <div>
